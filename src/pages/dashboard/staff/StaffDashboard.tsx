...
import { CalendarDays, Car, Users, Wrench, LogOut } from "lucide-react";
import { format } from "date-fns";
import { useNavigate } from "react-router-dom";
// import { getAppointmentsApi, updateAppointmentStatusApi } from "@/lib/appointmentApi"; // Temporarily disabled
  ...
  return (
    ...
    <div className="min-h-screen flex w-full">
        ...
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm text-muted-foreground">
              {user ? `Chào ${user.name}` : "Không tìm thấy thông tin người dùng"}
            </span>
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Quản lý lịch hẹn và khách hàng</h1>
        </div>
            ...
            <Table>
              ...
              <TableBody>
                ...
                <TableRow key={booking._id}>
                    <TableCell>
                      <div>
                      <div className="font-medium">{booking.user_id?.fullName || booking.user_id?.username || 'N/A'}</div>
                      <div className="text-sm text-muted-foreground">{booking.user_id?.email || 'N/A'}</div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="font-medium">{booking.vehicle_id?.brand} {booking.vehicle_id?.model || ''}</div>
                      <div className="text-sm text-muted-foreground">{booking.vehicle_id?.license_plate || 'N/A'}</div>
                    </TableCell>
                  <TableCell>{booking.center_id?.name || booking.center_id?.center_name || 'N/A'}</TableCell>
                    <TableCell>
                      <div>{booking.appoinment_date ? format(new Date(booking.appoinment_date), "dd/MM/yyyy") : '-'}</div>
                      <div className="text-sm text-muted-foreground">{booking.appoinment_time?.substring?.(0,5) || booking.appoinment_time || '-'}</div>
                    </TableCell>
                  <TableCell>{getStatusBadge(booking.status)}</TableCell>
                    <TableCell>
                      <div className="flex gap-2">
                      <Button size="sm" onClick={() => quickStatus()}>Nhận</Button>
                      <Button size="sm" variant="secondary" onClick={() => quickStatus()}>Hoàn tất</Button>
                      <Button size="sm" variant="destructive" onClick={() => quickStatus()}>Hủy</Button>
                      </div>
                    </TableCell>
                  </TableRow>
                  ))
